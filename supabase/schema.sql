...
-- ============================================
-- Single consolidated SQL file containing all database schema
-- for Grazel Apparel e-commerce platform
-- This file is idempotent - safe to run multiple times
-- Date: February 12, 2026
--
-- THIS IS THE ONLY SQL FILE NEEDED
-- All tables, indexes, policies, and functions are included here
--
-- FEATURES INCLUDED:
...
-- 11. Admin Settings Table (for return policy and other admin settings)
...
-- 12. Returns Table (for tracking return requests)
CREATE TABLE IF NOT EXISTS returns (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  order_id UUID NOT NULL REFERENCES orders(id) ON DELETE CASCADE,
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  reason TEXT NOT NULL,
  status TEXT DEFAULT 'requested' CHECK (status IN ('requested', 'approved', 'rejected', 'shipped', 'completed')),
...
-- Returns - users can view their own return requests
...
-- Admin Settings - Allow public read (for frontend to get return policy)
DROP POLICY IF EXISTS "Allow read admin settings" ON admin_settings;

CREATE POLICY "Allow read admin settings"
  ON admin_settings
  FOR SELECT
  USING (true);

CREATE POLICY "Allow insert admin settings"
  ON admin_settings
  FOR INSERT
  WITH CHECK (true);

CREATE POLICY "Allow update admin settings"
  ON admin_settings
  FOR UPDATE
  USING (true);

CREATE POLICY "Allow delete admin settings"
  ON admin_settings
  FOR DELETE
  USING (true);
...
-- Date: February 12, 2026
-- Version: FINAL - Single consolidated schema file
--
-- NOTE: This is the ONLY SQL file you need
-- All other schema files (schema-clean.sql, etc.) have been removed
-- Just use this single file for all database setup